...
// Mirrors assets/includes/functions_profanity.php on the PHP side: the base
// list still comes from Wo_Config('censored_words'), the "extra"/"removed"
// JSON files let admins go past that column's VARCHAR(20000) limit without
// restarting this process (re-read on every call; the files are tiny).
function loadProfanityJsonList(filePath) {
    try {
...
function getProfanityWordSet(ctx) {
    const base = (ctx.globalconfig.censored_words || '').split(',').map(w => w.trim()).filter(Boolean);
    const extra = loadProfanityJsonList(path.resolve(__dirname, '../../assets/data/censored_words_extra.json'));
    const removed = new Set(loadProfanityJsonList(path.resolve(__dirname, '../../assets/data/censored_words_removed.json')).map(w => w.toLowerCase()));
    const set = new Set();
    base.concat(extra).forEach(w => {
        const lower = (w || '').trim().toLowerCase();
        if (lower && !removed.has(lower)) {
            set.add(lower);
        }
    });
    return set;
}
 // Deriva los posibles singulares del token (igual que
 // Wo_ProfanitySingularCandidates() en PHP): la lista solo declara el
 // singular y aun asi "casinos" se detecta. ES + EN.
function profanityAccentLastVowel(stem) {
...
function profanitySingularCandidates(word) {
    ...
    if (word.endsWith('ves')) {
        out.push(word.slice(0, -3) + 'fe', word.slice(0, -3) + 'f');
    }
    if (word.endsWith('ces')) {
        out.push(word.slice(0, -3) + 'z');
    }
    if (word.endsWith('es')) {
        const stem = word.slice(0, -2);
        out.push(stem);
        const accented = profanityAccentLastVowel(stem);
        if (accented) {
            out.push(accented);
        }
    }
...
function profanityIsForbidden(lower, words) {
    if (words.has(lower)) {
        return true;
    }
    return profanitySingularCandidates(lower).some((c) => c && words.has(c));
}
 // Palabras CJK puras de mas larga a mas corta (igual que PHP): el
 // chino/japones/coreano no separa con espacios, van por subcadena.
function profanityCjkWords(words) {
    return [...words]
        .filter((w) => /^[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Hangul}]+$/u.test(w))
...
class FunctionsUtils {
    ...
    async Wo_CensoredWords(ctx, text) {
        if (text && typeof(text) != 'undefined' && text != '') {
            const words = getProfanityWordSet(ctx);
            if (words.size > 0) {
                // Same masking as PHP's Wo_ProfanityMask() / dowdes.com &
                // shomex.com.mx's ProfanityFilter::mask(): keep the first
                // floor(len/2) chars, mask the rest — "pendejo" -> "pen****".
                // Deteccion con paridad PHP: palabra completa + singulares
                // ES/EN + CJK por subcadena (ver functions_profanity.php).
                const mask = (token) => {
                    if (token.length <= 1) {
                        return '*'.repeat(Math.max(1, token.length));
                    }
                    const shown = Math.floor(token.length / 2);
                    return token.slice(0, shown) + '*'.repeat(token.length - shown);
                };
                text = text.replace(/\p{L}[\p{L}\p{N}']*/gu, (token) => {
                    return profanityIsForbidden(token.toLowerCase(), words) ? mask(token) : token;
                });
                for (const w of profanityCjkWords(words)) {
                    if (text.includes(w)) {
                        text = text.split(w).join(mask(w));
                    }
                }
            }
        }
