...
require_once dirname(__DIR__) . '/assets/includes/chismy_messenger_features.php';
if ($f == 'admin_setting' AND (Wo_IsAdmin() || Wo_IsModerator())) {
if ($s == 'save_chismy_messenger_features') {
        if (empty($_POST['hash_id']) || Wo_CheckSession($_POST['hash_id']) !== true) {
            echo json_encode(array('status' => 400, 'message' => 'Invalid session.'));
            exit();
        }
        $saved = ChismyMessengerSaveFeatures(
            isset($_POST['features']) && is_array($_POST['features'])
                ? $_POST['features']
                : array()
        );
        header('Content-Type: application/json');
        echo json_encode(array('status' => $saved ? 200 : 400));
exit();
}
if ($s == 'save_chismy_moderation_rule') {
    if (empty($_POST['hash_id']) || Wo_CheckSession($_POST['hash_id']) !== true) {
        header('Content-Type: application/json');
        echo json_encode(array('status' => 400, 'message' => 'Invalid session.'));
        exit();
    ...
    $reason = trim((string) ($_POST['reason'] ?? 'account_sale'));
    if ($phrase === '' || strlen($phrase) > 255) {
        header('Content-Type: application/json');
        echo json_encode(array('status' => 400, 'message' => 'La frase es obligatoria.'));
        exit();
    }
    ChismyModerationEnsureTables();
    $phrase = mysqli_real_escape_string($sqlConnect, $phrase);
    $locale = mysqli_real_escape_string($sqlConnect, $locale === '' ? '*' : $locale);
    $reason = mysqli_real_escape_string($sqlConnect, $reason === '' ? 'account_sale' : $reason);
    $ok = mysqli_query(
        $sqlConnect,
        "INSERT INTO `" . CHISMY_MODERATION_RULES_TABLE . "` (`phrase`,`reason`,`locale`,`enabled`,`created_by`,`created_at`) VALUES "
        . "('{$phrase}','{$reason}','{$locale}',1," . (int) $wo['user']['user_id'] . "," . time() . ") "
        . "ON DUPLICATE KEY UPDATE `reason` = '{$reason}', `enabled` = 1"
    );
    header('Content-Type: application/json');
    echo json_encode(array('status' => $ok ? 200 : 400));
    exit();
}
if ($s == 'toggle_chismy_moderation_rule') {
    if (empty($_POST['hash_id']) || Wo_CheckSession($_POST['hash_id']) !== true || empty($_POST['id'])) {
        header('Content-Type: application/json');
    ...
    $ok = mysqli_query($sqlConnect, "UPDATE `" . CHISMY_MODERATION_RULES_TABLE . "` SET `enabled` = {$enabled} WHERE `id` = {$rule_id}");
    header('Content-Type: application/json');
    echo json_encode(array('status' => $ok ? 200 : 400));
    exit();
}
if ($s == 'clear_chismy_moderation_user') {
    if (empty($_POST['hash_id']) || Wo_CheckSession($_POST['hash_id']) !== true || empty($_POST['user_id'])) {
        header('Content-Type: application/json');
        echo json_encode(array('status' => 400, 'message' => 'Invalid request.'));
        exit();
    }
    ...
    $ok = mysqli_query($sqlConnect, "UPDATE `" . CHISMY_MODERATION_USERS_TABLE . "` SET `violation_count` = 0, `blocked_until` = 0, `permanent` = 0, `last_reason` = '', `updated_at` = " . time() . " WHERE `user_id` = {$user_id}");
    header('Content-Type: application/json');
    echo json_encode(array('status' => $ok ? 200 : 400));
    exit();
}
if ($s == 'save_nsfw_settings') {
    if (empty($_POST['hash_id']) || Wo_CheckSession($_POST['hash_id']) !== true) {
        header('Content-Type: application/json');
        echo json_encode(array('status' => 400, 'message' => 'Invalid session.'));
        exit();
    }
    // Solo claves conocidas (Wo_SaveConfig solo hace UPDATE: ver SQL del zip).
    $allowed = array('nsfw_precheck_enabled', 'nsfw_block_score', 'nsfw_review_score', 'nsfw_text_block_hits', 'nsfw_cdn_tfjs', 'nsfw_cdn_nsfwjs', 'nsfw_api_url');
    foreach ($allowed as $key) {
        if (isset($_POST[$key])) {
            Wo_SaveConfig($key, trim((string) $_POST[$key]));
        }
    }
    header('Content-Type: application/json');
...
if ($s == 'nsfw_media_action') {
    ...
    $row_id = (int) $_POST['id'];
    $status = ($_POST['status'] === 'blocked') ? 'blocked' : 'approved';
    ChismyNsfwEnsureTable();
    $ok = mysqli_query($sqlConnect, "UPDATE `" . T_CHISMY_MEDIA_MOD . "` SET `status` = '{$status}' WHERE `id` = {$row_id}");
    if ($ok && $status === 'blocked') {
        $row_q = mysqli_query($sqlConnect, "SELECT `post_id`,`kind` FROM `" . T_CHISMY_MEDIA_MOD . "` WHERE `id` = {$row_id} LIMIT 1");
        $row = $row_q ? mysqli_fetch_assoc($row_q) : null;
        if (!empty($row['post_id']) && $row['kind'] === 'post') {
            $post_id = (int) $row['post_id'];
            mysqli_query($sqlConnect, "UPDATE `" . T_POSTS . "` SET `active` = '0' WHERE `id` = {$post_id}");
        }
    }
    header('Content-Type: application/json');
    echo json_encode(array('status' => $ok ? 200 : 400));
...
if ($s == 'download_report_evidence') {
        $report_id = isset($_GET['id']) ? (int) $_GET['id'] : 0;
        $table_check = mysqli_query($sqlConnect, "SHOW TABLES LIKE '" . T_REPORT_DETAILS . "'");
        $detail = ($report_id > 0 && $table_check && mysqli_num_rows($table_check) > 0)
            ? $db->where('report_id', $report_id)->getOne(T_REPORT_DETAILS)
            : null;
        $relative_path = !empty($detail->evidence_file) ? str_replace('\\', '/', $detail->evidence_file) : '';
        $reports_root = realpath(dirname(__DIR__) . '/upload/reports');
        $evidence_path = $relative_path !== '' ? realpath(dirname(__DIR__) . '/' . ltrim($relative_path, '/')) : false;

        if (!$reports_root || !$evidence_path || strpos($evidence_path, $reports_root . DIRECTORY_SEPARATOR) !== 0 || !is_file($evidence_path)) {
            http_response_code(404);
            exit('Evidence file not found.');
        }

        header('Content-Type: application/pdf');
        header('Content-Disposition: attachment; filename="report-evidence-' . $report_id . '.pdf"');
        header('Content-Length: ' . filesize($evidence_path));
    ...
    if ($s == 'ffmpeg_debug') {
        ...
        // Reverted to libx264 alongside FFMPEGUpload() — see functions_three.php.
        $shell                = shell_exec("$ffmpeg_b -y -i $video_file_full_path -vcodec libx264 -preset " . $wo['config']['convert_speed'] . " -crf 26 -filter:v scale=426:-2 $video_output_full_path_240 2>&1");
    ...
    if ($s == 'save_censored_words_extra' && Wo_CheckSession($hash_id) === true) {
        Wo_ProfanitySaveExtraWords(isset($_POST['words_text']) ? $_POST['words_text'] : '');
        resetCache();
        $data           = array();
        $data['status'] = 200;
        $data['count']  = Wo_ProfanityWordCount();
        header("Content-type: application/json");
        echo json_encode($data);
        exit();
    }
    ...
    if ($s == 'clear_all_cache' && Wo_CheckSession($hash_id) === true) {
        resetCache();
        $data           = array();
        $data['status'] = 200;
        header("Content-type: application/json");
    ...
    if ($s == 'optimize_png_assets' && Wo_CheckSession($hash_id) === true) {
        $data = array();
        if (!isfuncEnabled("shell_exec")) {
            $data['status'] = 200;
            $data['data']   = "La función shell_exec no está habilitada en este hosting, contacta a tu proveedor para poder usar oxipng.";
            header("Content-type: application/json");
            echo json_encode($data);
            exit();
        }
        $oxipng_bin = !empty($wo['config']['oxipng_binary_file']) ? $wo['config']['oxipng_binary_file'] : '';
        if (empty($oxipng_bin) || !file_exists($oxipng_bin)) {
            $data['status'] = 200;
            $data['data']   = "No se encontró el binario de oxipng en la ruta configurada. Descárgalo de https://github.com/oxipng/oxipng/releases, súbelo al servidor y guarda la ruta primero.";
            header("Content-type: application/json");
            echo json_encode($data);
            exit();
        }
        $count = 0;
        foreach (array('./themes', './admin-panel/assets') as $dir) {
            if (!is_dir($dir)) {
                continue;
            }
            ...
            foreach ($it as $file) {
                if (strtolower($file->getExtension()) === 'png') {
                    shell_exec(escapeshellarg($oxipng_bin) . ' -o 4 --strip safe ' . escapeshellarg($file->getPathname()) . ' 2>&1');
                    $count++;
                }
            }
        }
        $data['status'] = 200;
        $data['data']   = "Optimización completa. Archivos PNG procesados: {$count}";
        header("Content-type: application/json");
        echo json_encode($data);
        exit();
    }
    if ($s == 'change_admin_lang') {
        $new_lang = (!empty($_GET['lang']) && $_GET['lang'] === 'es') ? 'es' : 'en';
        setcookie("admin_panel_lang", $new_lang, time() + (10 * 365 * 24 * 60 * 60), '/');
    }
    if ($s == 'change_mode') {
