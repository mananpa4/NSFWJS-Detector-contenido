...
if ($f == 'posts') {
    ...
    if ($s == 'insert_new_post') {
        ...
        // Chismy NSFW: texto (bloqueo servidor) + veredicto NSFWJS del navegador.
        $chismy_nsfw = ChismyNsfwEvaluate($post_text, isset($_POST['chismy_nsfw']) ? $_POST['chismy_nsfw'] : null);
        if ($chismy_nsfw['action'] === 'block') {
            ChismyNsfwJsonBlock($chismy_nsfw['message']);
        }
        $chismy_nsfw_review = ($chismy_nsfw['action'] === 'review');
        ...
        if (empty($errors) && $invalid_file == false) {
            ...
            $post_active = 1;
            if (($wo['config']['post_approval'] == 1 && !Wo_IsAdmin()) || !empty($chismy_nsfw_review)) {
                $post_active = 0;
            }
        ...
        if (!empty($id)) {
            // Chismy NSFW: rastro de media para la cola de revision (admin).
            $chismy_nsfw_files = array();
            if (!empty($mediaFilename)) {
                $chismy_nsfw_files[] = $mediaFilename;
            }
            if (isset($_FILES['postPhotos']['name']) && is_array($_FILES['postPhotos']['name'])) {
                foreach ($_FILES['postPhotos']['name'] as $chismy_nsfw_name) {
                    $chismy_nsfw_files[] = (string) $chismy_nsfw_name;
                }
            }
            ChismyNsfwLogMedia($wo['user']['user_id'], $id, 'post', $chismy_nsfw_files, $chismy_nsfw);
            if ($is_reel == 1) {
    ...
    if ($s == 'register_comment') {
        if (!empty($_POST['post_id']) && isset($_POST['text'])) {
            ...
            // Chismy NSFW: bloqueo por palabras prohibidas (servidor, confiable).
            $chismy_comment_nsfw = ChismyNsfwEvaluate($text_comment, null);
            if ($chismy_comment_nsfw['action'] === 'block') {
                ChismyNsfwJsonBlock($chismy_comment_nsfw['message']);
            }
            $C_Data = array(
                'user_id' => Wo_Secure($wo['user']['user_id']),
                'page_id' => Wo_Secure($page_id),
            ...
            // Chismy NSFW: rastro de la imagen del comentario (si trae).
            ChismyNsfwLogMedia($wo['user']['user_id'], (int) $_POST['post_id'], 'comment', array($comment_image), $chismy_comment_nsfw);
    ...
    if ($s == 'delete_comment') {
        // Esta respuesta siempre debe llevar un `status`. Cuando faltaba, el
        // cliente se quedaba con el spinner girando porque solo reacciona a 200.
        $data = array(
            'status' => 400
        );
        if (empty($_GET['comment_id'])) {
            $data['error'] = 'missing_comment_id';
        } else if (Wo_CheckMainSession($hash_id) !== true) {
            $data['status'] = 403;
            $data['error']  = 'invalid_session';
        } else {
            $comment_id = Wo_Secure($_GET['comment_id']);
            try {
                // La eliminación debe terminar antes de responder. La respuesta
                // anticipada de Wo_RunInBackground dejaba el navegador esperando
                // una operación que PHP/LiteSpeed podía finalizar prematuramente.
                if (Wo_DeletePostComment($comment_id) === true) {
                    $data = array(
                        'status' => 200
                    );
                } else {
                ...
                // PHP 8 convierte los errores de mysqli en excepciones. Sin este
                // catch la peticion moria con un 500 y el modal no se cerraba.
                $data['status'] = 500;
                $data['error']  = 'delete_failed';
                error_log('delete_comment(' . $comment_id . '): ' . $exception->getMessage());
            }
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    if ($s == 'report_post') {
        if (!empty($_POST['enhanced_report']) && !empty($_POST['post_id'])) {
            $post_id = (int) $_POST['post_id'];
            $category = trim((string) ($_POST['category'] ?? ''));
            $subcategory = trim((string) ($_POST['subcategory'] ?? ''));
            $details = trim((string) ($_POST['details'] ?? ''));
            $allowed_categories = array(
                'minor', 'harassment', 'self_harm', 'violence', 'restricted',
                'adult', 'fraud', 'intellectual_property'
            );
            $details_table_result = mysqli_query($sqlConnect, "SHOW TABLES LIKE '" . T_REPORT_DETAILS . "'");
            if (!$details_table_result || mysqli_num_rows($details_table_result) === 0) {
                $data = array('status' => 503, 'message' => 'El sistema de reportes necesita instalar su actualizacion de base de datos.');
            } elseif (!in_array($category, $allowed_categories, true) || empty($subcategory)) {
                $data = array('status' => 400, 'message' => 'Selecciona una categoria y una opcion valida.');
            } elseif (mb_strlen($details, 'UTF-8') < 200) {
                $data = array('status' => 400, 'message' => 'El reporte debe tener al menos 200 caracteres.');
            } else {
                $normalized = mb_strtolower(preg_replace('/\s+/u', ' ', $details), 'UTF-8');
                preg_match_all('/[\p{L}\p{N}]+/u', $normalized, $word_matches);
                $words = $word_matches[0];
                $word_count = count($words);
                $frequencies = $word_count ? array_count_values($words) : array();
                $max_frequency = $frequencies ? max($frequencies) : 0;
                $looks_like_spam = preg_match('/(.)\1{7,}/u', $normalized) === 1 ||
                    ($word_count >= 12 && ($max_frequency / $word_count) > 0.45) ||
                    ($word_count >= 20 && (count($frequencies) / $word_count) < 0.18);
                if ($looks_like_spam) {
                    $data = array(
                        'status' => 422,
                        'message' => 'El reporte parece spam y fue rechazado automaticamente.'
                    );
                } elseif (Wo_IsPostRepotred($post_id, $wo['user']['user_id'])) {
                    $data = array('status' => 409, 'message' => 'Ya reportaste esta publicacion.');
                } else {
                    $evidence_path = '';
                    if (!empty($_FILES['evidence']['tmp_name'])) {
                        ...
                        if ($category !== 'intellectual_property' ||
                            ...
                            $data = array(
                                'status' => 400,
                                'message' => 'El comprobante debe ser un PDF valido de hasta 1 MB.'
                            );
                        } else {
                            $directory = 'upload/reports/' . date('Y/m');
                            $absolute_directory = dirname(__DIR__) . '/' . $directory;
                            if (!is_dir($absolute_directory)) {
                                @mkdir($absolute_directory, 0755, true);
                            }
                            $filename = bin2hex(random_bytes(16)) . '.pdf';
                            $absolute_path = $absolute_directory . '/' . $filename;
                            if (!move_uploaded_file($file['tmp_name'], $absolute_path)) {
                                $data = array('status' => 500, 'message' => 'No se pudo guardar el comprobante.');
                            } else {
                    ...
                    if (empty($data)) {
                        $result = Wo_ReportPost(array('post_id' => $post_id));
                        if ($result === 'report') {
                            $report = $db->where('user_id', $wo['user']['user_id'])
                                ->where('post_id', $post_id)->orderBy('id', 'DESC')->getOne(T_REPORTS);
                            if (!empty($report->id)) {
                                $db->insert(T_REPORT_DETAILS, array(
                                    'report_id' => $report->id,
                                    'reporter_id' => $wo['user']['user_id'],
                                    'target_type' => 'post',
                                    'category' => Wo_Secure($category),
                                    'subcategory' => Wo_Secure($subcategory),
                                    'details' => Wo_Secure($details, 0),
                                    'evidence_file' => Wo_Secure($evidence_path),
                                    'status' => 'pending',
                                    'created_at' => time()
                                ));
                            }
                            $data = array('status' => 200, 'message' => 'Gracias por avisarnos. Revisaremos tu reporte.');
                        } else {
                            $data = array('status' => 500, 'message' => 'No se pudo registrar el reporte.');
                        }
                    }
                }
            }
